...
echo -e "  ${GREEN}✓${NC} config/rhino.yml"

# ============================================================
# Phase 2: Generate config/evals/beliefs.yml
# ============================================================
...
#
# Types:
#   file_check    — file exists, contains string (mechanical)
#   content_check — forbidden words in source dirs (mechanical)
#
# Generated by: rhino init
...
# rhino.yml has hypothesis
cat <<'EOF'

  - id: rhino-config-exists
    belief: "rhino.yml has value hypothesis"
    type: file_check
...
HAS_TS=false
[[ -f "tsconfig.json" || -f "apps/web/tsconfig.json" ]] && HAS_TS=true

if [[ "$PROJECT_TYPE" == "nextjs" || "$PROJECT_TYPE" == "react" || "$PROJECT_TYPE" == "vue" || "$PROJECT_TYPE" == "svelte" ]]; then
    if [[ "$HAS_TS" == true && -n "$SRC_DIR" ]]; then
        cat <<'EOF'

  # ── hygiene ────────────────────────────────────────

  - id: no-console-log
    belief: "No console.log in source"
    type: content_check
  ...
  - id: no-any-types
    belief: "No 'any' type annotations"
    type: content_check
    forbidden: [": any", "as any"]
    feature: hygiene
    severity: warn
EOF
...
# TODO/FIXME check for all projects with source dirs
if [[ -n "$SRC_DIR" ]]; then
    cat <<'EOF'

  - id: no-todo-markers
    belief: "No TODO/FIXME/HACK markers left in source"
    type: content_check
    forbidden: ["TODO", "FIXME", "HACK"]
    feature: hygiene
    severity: warn
EOF
    ASSERTION_COUNT=$((ASSERTION_COUNT + 1))
...
if [[ ${#FEATURES[@]} -gt 0 ]]; then
    ...
    for i in "${!FEATURES[@]}"; do
        ...
        # Determine the right file check for this feature
        if [[ "$feat_path" == app/\(* ]]; then
            # Next.js route group — check for page.tsx
            page_file="${feat_path}page.tsx"
            [[ ! -f "$page_file" ]] && page_file="${feat_path}page.ts"
            [[ ! -f "$page_file" ]] && page_file="${feat_path}page.jsx"
            [[ ! -f "$page_file" ]] && page_file="${feat_path}page.js"
            # Even if no page exists yet, assert the directory
            cat <<EOF

  - id: feature-${feat}-exists
    belief: "${feat} route group exists"
    type: file_check
    path: "${feat_path}"
    feature: ${feat}
    severity: warn
EOF
            ...
            if [[ -f "${feat_path}package.json" ]]; then
                # Workspace package
                cat <<EOF

  - id: feature-${feat}-exists
    belief: "${feat} workspace has package.json"
    type: file_check
    path: "${feat_path}package.json"
    feature: ${feat}
    severity: warn
EOF
            else
...
EOF
            ...
            # File-based feature (script)
            cat <<EOF

  - id: feature-${feat}-exists
    belief: "${feat} script exists"
    type: file_check
    path: "${feat_path}"
    feature: ${feat}
    severity: warn
...
echo -e "  ${GREEN}✓${NC} config/evals/beliefs.yml (${ASSERTION_COUNT} assertions)"
